...
public class VlvXXlClient implements ClientModInitializer {
    ...
    public static boolean killAuraPvE = true;
    public static boolean killAuraPvP = false;
    ...
    public static double reachPvE = 4.5;
    public static double reachPvP = 3.05;
    ...
    private static final int MENU_H = 200;
    ...
    private void onHud(DrawContext ctx, RenderTickCounter tickCounter) {
        ...
        if (esp && mc.world != null) drawEsp(ctx, mc);
        if (!menuOpen) return;
        ...
        if (dragging) {
            if (leftDown) { menuX = mx - dragX; menuY = my - dragY; }
            else dragging = false;
        }
        ...
        int y = menuY + 28;
        y = toggleButton(ctx, mc, mx, my, leftDown, "KillAura PvE", killAuraPvE, menuX + 6, y, 0);
        y = toggleButton(ctx, mc, mx, my, leftDown, "KillAura PvP", killAuraPvP, menuX + 6, y, 1);
        y = toggleButton(ctx, mc, mx, my, leftDown, "ESP",          esp,        menuX + 6, y, 2);
    ...
    private void drawEsp(DrawContext ctx, MinecraftClient mc) {
        ...
        for (Entity ent : mc.world.getEntities()) {
            ...
            double dy = ent.getY() + ent.getHeight() / 2.0 - cam.y;
    ...
    private int toggleButton(DrawContext ctx, MinecraftClient mc, int mx, int my, boolean leftDown,
        ...
        if (hover && leftDown) {
            switch (id) {
                case 0 -> killAuraPvE = !killAuraPvE;
                case 1 -> killAuraPvP = !killAuraPvP;
                case 2 -> esp = !esp;
    ...
    private void attack(Entity target, MinecraftClient mc) {
    ...
    private void onTick(MinecraftClient mc) {
        ...
        if (killAuraPvE) {
            Entity target = null;
            double best = reachPvE;
            for (Entity ent : mc.world.getEntities()) {
                if (!(ent instanceof MobEntity) || ent == mc.player) continue;
                double d = mc.player.distanceTo(ent);
                if (d < best) { best = d; target = ent; }
            }
            if (target != null) attack(target, mc);
        }

        if (killAuraPvP) {
            Entity target = null;
            double best = reachPvP;
            for (Entity ent : mc.world.getEntities()) {
                if (!(ent instanceof PlayerEntity) || ent == mc.player) continue;
                if (((PlayerEntity) ent).isCreative()) continue;
                double d = mc.player.distanceTo(ent);
                if (d < best) { best = d; target = ent; }
            }
            if (target != null) attack(target, mc);
        ...
        if (antiBan) {
            ...
            if (suspicion > 25) { killAuraPvP = false; suspicion = 0; }
